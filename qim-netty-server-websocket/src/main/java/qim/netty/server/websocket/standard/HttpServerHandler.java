package qim.netty.server.websocket.standard;
...
import qim.netty.server.websocket.support.WsPathMatcher;
...
class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (!req.headers().contains(HttpHeaderNames.UPGRADE) || !req.headers().contains(HttpHeaderNames.SEC_WEBSOCKET_KEY) || !req.headers().contains(HttpHeaderNames.SEC_WEBSOCKET_VERSION)) {
            if (forbiddenByteBuf != null) {
                res = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.FORBIDDEN, forbiddenByteBuf.retainedDuplicate());
            } else {
                res = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.FORBIDDEN);
